...
## 接收流的状态(Receiving Stream States)
...
## 流终止处理(Handling Stream Cancellation)
...
### 发布连接 ID（Issuing Connection IDs）
...
### 消费和收回连接ID(Consuming & Retiring Connection IDs
...
## 加密与传输握手(Cryptographic and Transport Handshake)
...
`0-RTT 握手（Handshake）的示例`

## 协商连接IDs （Negotiating Connection IDs）
...
## 连接建立过程中的地址验证(Address Validation During Connection Establishment)
...
### 使用重试包进行地址验证(Address Validation using Retry Packets)
...
### 链路上的地址欺骗(On-Path Address Spoofing)
...
### 链路外的包导向(Off-Path Packet Forwarding)
...
## 服务端首选地址（Server's Preferred Address）
...
## IPv6流标签与迁移的使用(Use of IPv6 Flow-Label and Migration)
...
## 关闭中和释放中状态（Closing and Draining Connection States）
...
`无状态重置令牌(Stateless Reset Packet)`
...
## 包处理和确认（Packet Processing and Acknowledgment）
...
## ICMP 包太大消息(ICMP Packet Too Big Messages)
...
到 Github wiki，地址为 <https://github.com/quicwg/base-drafts/wiki/QUIC-Versions>.

## 可变长度数字编码(Variable-Length Integer Encoding)
...
## 包编号的编码和解码（Packet Number Encoding and Decoding）
...
## 传输参数编码(Transport Parameter Encoding)
...
## 传输参数定义(Transport Parameter Definitions)
...
`首选地址格式(Preferred Address format)`
...
## MAX_DATA Frame帧(MAX_DATA Frame)
...
## MAX_STREAM_DATA帧 (MAX_STREAM_DATA Frame)
...
## MAX_STREAMS(MAX_STREAMS Frames)
...
## DATA_BLOCKED帧(DATA_BLOCKED Frame)
...
## 流数据阻塞帧(STREAM_DATA_BLOCKED Frame)
...
## 新连接ID帧(NEW_CONNECTION_ID Frame) 
...
## 销毁连接ID帧(RETIRE_CONNECTION_ID Frame)
...
## PATH_CHALLENGE帧(PATH_CHALLENGE Frame)
...
## PATH_RESPONSE帧 (PATH_RESPONSE Frame) 
...
## CONNECTION_CLOSE 帧 (CONNECTION_CLOSE Frames)
...
## 应用协议错误码（Application Protocol Error Codes)
...
## 无状态重置Oracle(Stateless Reset Oracle)
...
## QUIC传输参数注册信息(QUIC Transport Parameter Registry)
...
## QUIC 帧种类注册处(QUIC Frame Type Registry)
...
## QUIC 传输错误码注册处(QUIC Transport Error Codes Registry)
...
`初始QUIC 传输错误码条目(Initial QUIC Transport Error Codes Entries)`
