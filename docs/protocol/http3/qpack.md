...
- 编码器使用单向流来修改动态表的状态，不会发出与任何特定请求相关联的头字段。

- 请求和推送流中的HEADERS和PUSH_PROMISE帧在不修改表状态的情况下引用表．

- 解码器在单向流上向编码器发送反馈。这个反馈使编码器能够管理动态表状态。
...
`耗尽动态表项(Draining Dynamic Table Entries)`

### 避免报头块阻塞 (Avoiding Head-of-Line Blocking)
...
但与其他解码器的反馈一起可能是冗余的。通过延迟插入计数增量指令，解码器可能能够合并多个插入计数增量指令，或者用报头确认完全替换它们(参见 `header-acknowledgement`)。
...
### 设置动态表容量(Set Dynamic Table Capacity)
